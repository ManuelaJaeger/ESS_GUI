classdef level1Study < levelStudy;
    ...
    %
    % update: 
    % 03.02.2017: Manuela Jaeger, copy .fdt file if .set EEGLAB file is
    %             provided
    % 03.02.2017: Manuela Jaeger, copy .vmrk and .eeg brainvision recorder
    %             file if .vhdr is provided, adapt header and marker file
    %             to fit file naming convention
    % 28.02.2017: Manuela Jaeger, added capId information to the sessionTaskInfo
    ...
    properties
        ...
        sessionTaskInfo = struct('sessionNumber', ' ', 'taskLabel', ' ', 'labId', ' ',...
            ...
            'hearing', ' ', 'height', ' ', 'weight', ' ', 'channelLocations', ' ', 'capId', ' ',...
                            ...
                            if exist(fileFinalPath, 'file')
                                ...
                                if overwriteFiles || ~exist(destinationFile, 'file')
                                    if copyfile(fileFinalPath, destinationFile)
                                        ...
                                        %copy .fdt file if eeglab .set file
                                        %is provided
                                        if strcmp(extension,'.set')
                                            [path, name, dummy] = fileparts(fileFinalPath);
                                            fileFinalPath_fdt= [path filesep name '.fdt'];
                                            filenameInEss_fdt = obj.essConventionFileName(namePrefixString, obj.studyTitle, obj.sessionTaskInfo(i).sessionNumber,subjectInSessionNumber, obj.sessionTaskInfo(i).taskLabel, j, getSubjectLabIdForDataRecording(obj, i, j), length(obj.sessionTaskInfo(i).subject), name, '.fdt');                                
                                            destinationFile_fdt = [essFolder filesep essConventionfolder filesep filenameInEss_fdt];
                                            copyfile(fileFinalPath_fdt,destinationFile_fdt);
                                            fprintf('File %s copied successfully to %s.\n', fileFinalPath_fdt, destinationFile_fdt);
                                        end
                                        
                                        %copy .vmrk and .eeg file if
                                        %brainvision .vhdr file is provided
                                        if strcmp(extension,'.vhdr')
                                            [path, name, dummy] = fileparts(fileFinalPath);
                                            %copy and rename .vmrk file
                                            fileFinalPath_vmrk= [path filesep name '.vmrk'];
                                            filenameInEss_vmrk = obj.essConventionFileName(namePrefixString, obj.studyTitle, obj.sessionTaskInfo(i).sessionNumber,subjectInSessionNumber, obj.sessionTaskInfo(i).taskLabel, j, getSubjectLabIdForDataRecording(obj, i, j), length(obj.sessionTaskInfo(i).subject), name, '.vmrk');                                
                                            destinationFile_vmrk = [essFolder filesep essConventionfolder filesep filenameInEss_vmrk];
                                            copyfile(fileFinalPath_vmrk,destinationFile_vmrk);
                                            fprintf('File %s copied successfully to %s.\n', fileFinalPath_vmrk, destinationFile_vmrk);
                                            
                                            %copy and rename .eeg file
                                            fileFinalPath_eeg= [path filesep name '.eeg'];
                                            filenameInEss_eeg = obj.essConventionFileName(namePrefixString, obj.studyTitle, obj.sessionTaskInfo(i).sessionNumber,subjectInSessionNumber, obj.sessionTaskInfo(i).taskLabel, j, getSubjectLabIdForDataRecording(obj, i, j), length(obj.sessionTaskInfo(i).subject), name, '.eeg');                                
                                            destinationFile_eeg = [essFolder filesep essConventionfolder filesep filenameInEss_eeg];
                                            copyfile(fileFinalPath_eeg,destinationFile_eeg);
                                            fprintf('File %s copied successfully to %s.\n', fileFinalPath_eeg, destinationFile_eeg); 
                                            
                                            
                                            %Brainvision recoder header and 
                                            %marker files have an internal
                                            %link which points to the
                                            %corresponding eeg and marker
                                            %file. Both files need to be
                                            %changed to fit the new file
                                            %naming convention - this needs
                                            %to be adjusted if
                                            %Brainvision changes the main
                                            %structure of the header and
                                            %marker files
                                            
                                            %change Brainvision .vhdr header file to fit new filenames
                                            [fileID message]=fopen(destinationFile,'r');
                                            y=textscan(fileID,'%s','delimiter','\n');
                                            fclose(fileID);
                                            %change DataFile and MarkerFile in Line 6 and 7                                            
                                            y{1,1}{6,1}=(['DataFile=' filenameInEss_eeg]); 
                                            y{1,1}{7,1}=(['MarkerFile=' filenameInEss_vmrk]);
                                            [rows,cols] = size(y{1,1});
                                            [fileID message]=fopen(destinationFile,'w');
                                            for row=1:rows
                                                fprintf(fileID,'%s\n',y{1,1}{row,1});
                                            end
                                            fclose(fileID);
                                            
                                            %change Brainvision .vmrk marker file
                                            %to fit new filenames
                                            [fileID message]=fopen(destinationFile_vmrk,'r');
                                            y=textscan(fileID,'%s','delimiter','\n');
                                            fclose(fileID);
                                            %change DataFile in Line 5                                           
                                            y{1,1}{5,1}=(['DataFile=' filenameInEss_eeg]);                                            
                                            [rows,cols] = size(y{1,1});
                                            [fileID message]=fopen(destinationFile_vmrk,'w');
                                            for row=1:rows
                                                fprintf(fileID,'%s\n',y{1,1}{row,1});
                                            end
                                            fclose(fileID); 
